...
Table.prototype.delete = function (hashKey, rangeKey, options, callback) {
  var self = this;
  if (typeof options === 'function' && !callback) {
    callback = options;
    options = {};
  ...
  var start = function (callback) {
    var paramName = _.isString(self.schema.deletedAt) ? self.schema.deletedAt : 'deletedAt';

    if(self.schema.timestamps && self.schema.deletedAt !== false && !_.has(item, paramName)) {
      item[paramName] = new Date().toISOString();
    }
