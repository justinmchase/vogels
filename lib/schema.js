...
var Schema = module.exports = function (config) {
  ...
  Joi.validate(config, internals.configSchema, { context: context }, function (err, data) {
    ...
    if(self.timestamps) {
      ...
      var deletedAtParamName = 'deletedAt';
      ...
      if(self.createdAt) {
        if(_.isString(self.deletedAt)) {
          deletedAtParamName = self.deletedAt;
        }
      }
      ...
      if(self.deletedAt !== false) {
        valids[deletedAtParamName] = Joi.date();
      }
